...
def _ewma_window_stats(
    ...
    """指数加权窗口统计（主流实践: 输入端平滑，2026-08-15）。

    共享时钟 EWMA: 每笔 decisive 交易（pnl≠0）推进一次衰减，胜/负
    累加器同步衰减后累加新观测；保本交易跳过（不推进时钟，与
    compute_kelly 剔除保本的口径一致）。pnls 必须按时间升序（旧→新）。

    相比平权窗口的作用（对齐波动率目标领域的 EWMA λ=0.94 惯例）：
      - 消除窗口边界驱逐跳变（旧交易渐隐而非整笔移出）
      - 体制切换响应更快（新边权重更高）
    代价是有效样本量缩短（≈ (1+λ)/(1−λ)），故默认 λ=0.97（有效记忆
    ≈65 笔内取 33 笔口径），而非波动率场景的 0.94（那是按日频标定的）。

    Returns:
        (win_rate, avg_win, avg_loss) — 与 compute_kelly 平权口径同构
    """
    sw = 0.0   # 指数加权胜金额和
    nw = 0.0   # 指数加权胜笔数（权重和）
    sl = 0.0   # 指数加权负金额和
...
def _kelly_f_from_stats(win_rate: float, avg_win: float, avg_loss: float) -> float:
    ...
    抽出为独立函数使 EWMA 路径与平权路径共用同一条裁剪链，防止两条
    路径的边界行为漂移（单一事实源）。
    """
    if avg_loss == 0:
        avg_loss = 1.0
...
def rolling_kelly_risk_pct(
    ...
    """滚动 Kelly 风险上限 (2026-08-15 小资金翻倍协议: 探索→利用)。

    从 StateManager.recent_pnl（最近 100 笔已实现盈亏，USDT）滚动计算
    单笔风险比例上限。与 compute_kelly 的等价性已由 test_rolling_kelly.py
    证明：窗口切片 == 同窗口全量计算，切片本身零偏差。

    样本量规则（与 test_rolling_kelly.TestSampleSizeRuleContract 契约一致）：
      - decisive 样本(pnl≠0) < min_samples(默认 10): 返回 None —
        数据不足不引入噪声仓位的假精度，调用方保持现有 eff_risk
      - decisive < sample_full_kelly(默认 50): 1/4 Kelly（探索档）
      - decisive >= sample_full_kelly: 1/2 Kelly（利用档，业界标准）
      - 结果裁剪到 [min_risk_pct, max_risk_pct]（默认 [1.0, base]）；
        负边 Kelly=0 时落到 min_risk_pct 而非 0（暂停由期望值门禁负责，
        这里保证 sizing 有合法下限，防御 minSz 取整后归零的假单）

    EWMA 平滑（2026-08-15 主流实践: 输入端平滑）：
      - ewma_lambda ∈ (0,1): 窗口统计按共享时钟指数加权，消除窗口
        边界驱逐跳变、加快体制切换响应；默认 0.97
      - ewma_lambda <= 0 / >= 1 / 非法值: 回退平权窗口（与旧版逐位一致）

    数学依据: g(f)=p·ln(1+f·b)+q·ln(1−f)，f*>2×f* 处增长归零，
    分数 Kelly 牺牲部分增长换取回撤减半（半 Kelly ≈ 75% 增长）。

    Args:
        recent_pnl: 滚动已实现盈亏列表（USDT，符号即盈亏，时间升序）
        base_risk_pct: 基准风险比例（config risk.risk_per_trade_pct），
            同时作为默认上限
        risk_cfg: config 的 risk 子字典（读 rolling_kelly 配置块）

    Returns:
        (risk_pct 或 None, 诊断 dict)；None 表示不施加 Kelly 约束
    """
    ...
    try:
        ...
        # 惯例与 max_daily_trades/max_position_leverage 一致: 0 = 用基准值
    ...
    if lam > 0.0:
        # EWMA 路径: 指数加权统计 + 共用裁剪链（_kelly_f_from_stats）
        ...
        # 平权路径（λ 未配置/非法）: 复用 compute_kelly
        # （pnl 直接作 pnl_pct 口径，分类按符号、量级按绝对值）
        trades = [TradeRecord(symbol="ROLLING", direction="long",
                              entry_time=0.0, pnl=p, pnl_pct=p) for p in pnls]
        r = compute_kelly(trades)
        kelly_f = r.kelly_fraction
        win_rate_r = r.win_rate
        avg_win_r = r.avg_win_pct
        avg_loss_r = r.avg_loss_pct

    tier = "half" if decisive >= sample_full else "quarter"
    frac = 0.5 if tier == "half" else 0.25
    risk = kelly_f * frac * 100.0
    # 裁剪: 上限 max_risk，下限 min(min_risk, max_risk) 防坏配置反超
