...
### 背景（穿仓事故第二层根因）
PUMP 第三笔 50x 多单 SL=-5.09% 而理论爆仓点仅 -2%：止损距离超过爆仓距离，
价格先爆仓后止损。此前 `liq_check_fail_closed=false` 对"止损≥爆仓安全距离"
仅警告放行，假设"逐仓爆仓只损保证金"；但 paper 引擎当时无爆仓行为，
放行 = 无限亏损敞口（单笔 -28.85 穿仓）。

### 修改（executor.execute_signal LiqCheck）
- 命中 `止损距离 ≥ 爆仓距离×安全系数` 且非 fail-closed 时，**降杠杆**使
  爆仓距离扩至覆盖止损距离：`新杠杆 = floor(1/(止损距离/安全系数 + MMR))`
- 写回 `signal.leverage`（实盘下单 + paper 开仓 + 自适应杠杆 finally 恢复前
  均取到降杠杆值），止损价不变（不牺牲止损宽度）
- 数学保证：`(1/new_lev − MMR) × safety ≥ stop_dist`，止损必然先于爆仓触发，
  单笔亏损 ≤ 保证金
- fail-closed=true 或降杠杆无效（新杠杆 ≥ 当前杠杆）仍拒单；杠杆非法仍拒单

### 验证
- 新增 3 个单测：满倍率降杠杆 50x→15x（3%止损）、50x→9x（5%止损，SL 不变）、
  做空方向对称降杠杆；回归 51/51 全过
- 重启 agent 生效
