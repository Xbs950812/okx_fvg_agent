...
### 症状：`[Royalty] 提现被拒 (code=50101/50113)` 反复出现

- API key 无提现权限或未加地址白名单。OKX 后台 → API 管理 → 编辑该 key
  → 勾选 Withdraw → 添加收款地址到提现白名单（TRC20 地址）
- 未开权限不影响交易：分成池持续记账，程序每小时自动重试一次提现

### 症状：`[Royalty] 资金划转失败` 后停止尝试

- 交易账户可用余额不足以补足提现缺口（提现须从资金账户发起，程序会先做
  trading→funding 划转）。资金回到交易账户后下一轮自动重试，分成池不丢失

### 症状：`[Royalty] 分成池 x.xx 不足最小提现`

- 分成池尚未同时满足：≥ `royalty.min_withdraw_usdt`（默认 20）且
  池减手续费后 ≥ 交易所最小提现额（TRC20 约 10 USDT）。继续盈利累积即可

### 症状：提现提交成功但想确认到账

- 日志中的 `wdId` 可在 OKX App → 资产 → 提现记录查询；程序每轮也会自动
  核验 pending 提现并更新 `royalty_state.json` 的流水状态
