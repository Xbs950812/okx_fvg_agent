...
def _estimate_funding_cost(
    ...
    # 结算次数（每 8 小时一次）
    funding_cycles = max(1, int(hold_hours / 8))
...
def _estimate_fee_cost(
    ...
    position_value = pos_size * ct_val * avg_px
    fee_rate = 0.0005 if is_taker else 0.0002  # taker 0.05%, maker 0.02%
    # 开仓 + 平仓各一次
...
def _evaluate_market_guard(
    ...
    try:
        ...
        # BTC 持仓量变化（24h）— 需要历史 OI 基准，无历史数据时禁用
        oi_change_pct = 0.0
        try:
            oi_current = client.get_open_interest("BTC-USDT-SWAP")
            if oi_current is not None and hasattr(market_guard, '_prev_oi') and market_guard._prev_oi > 0:
                oi_change_pct = (oi_current - market_guard._prev_oi) / market_guard._prev_oi
            # 保存当前 OI 作为下轮基准
...
def main_loop(config: dict, once: bool = False, max_rounds: int = 0):
    ...
    while True:
            ...
            try:
                ...
                if paper_engine is not None:
                    for _pinst, _ppos in paper_engine._positions.items():
                        if _ppos.filled and _pinst not in _FILL_FUNNEL["counted_filled"]:
                            _FILL_FUNNEL["counted_filled"].add(_pinst)
        ...
        _orig_fvg_cfg = scan_config.get("strategy", {}).get("min_fvg_width_pct", {})
        ...
        if inst_info is None:
            ...
            try:
                if adaptive_tuner:
                    eff_lev, eff_risk, _ = adaptive_tuner.get_effective_params()
                    ...
                    # 使用配置副本应用自适应风险比例，避免修改共享 config
                    if eff_risk != risk_cfg["risk_per_trade_pct"]:
